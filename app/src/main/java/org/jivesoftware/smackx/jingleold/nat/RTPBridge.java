...
public class RTPBridge extends IQ {
    ...
    public static class Provider extends IQProvider<RTPBridge> {
        ...
        @Override
        public RTPBridge parse(XmlPullParser parser, int initialDepth) throws Exception {
    ...
    public static boolean serviceAvailable(XMPPConnection connection) throws NoResponseException,
        ...
        ServiceDiscoveryManager disco = ServiceDiscoveryManager
            ...
            Iterator iter = (Iterator) items.getItems();
            while (iter.hasNext()) {
                DiscoverItems.Item item = (DiscoverItems.Item) iter.next();
                if (item.getEntityID().toString().startsWith("rtpbridge.")) {
                    return true;
                }
            }
