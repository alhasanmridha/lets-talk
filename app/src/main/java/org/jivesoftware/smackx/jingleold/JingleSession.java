...
public final class JingleSession extends JingleNegotiator implements MediaReceivedListener {
    ...
    public synchronized void receivePacketAndRespond(IQ iq) throws XMPPException, SmackException, InterruptedException {
        ...
        LOGGER.fine("Packet: " + iq.toXML(iq.getChildElementNamespace()));
                ...
                if (packet instanceof IQ) {
                    ...
                    if (iq instanceof Jingle) {
                        ...
                        if (sid == null || !sid.equals(getSid())) {
                            LOGGER.fine("Ignored Jingle(SID) " + sid + "|" + getSid() + " :" + iq.toXML(iq.getChildElementNamespace()));
                            return false;
                        }
                        Jid ini = jin.getInitiator();
                        if (!ini.equals(getInitiator())) {
                            LOGGER.fine("Ignored Jingle(INI): " + iq.toXML(iq.getChildElementNamespace()));
                            return false;
                        }
                    } else {
                        // We accept some non-Jingle IQ packets: ERRORs and ACKs
                        if (iq.getType().equals(IQ.Type.set)) {
                            LOGGER.fine("Ignored Jingle(TYPE): " + iq.toXML(iq.getChildElementNamespace()));
                            return false;
    ...
    public IQ createJingleError(IQ iq, JingleError jingleError) {
        ...
        if (jingleError != null) {
            ...
            LOGGER.severe("Error sent: " + errorPacket.toXML(errorPacket.getChildElementNamespace()));
        }
        return errorPacket;
    }
